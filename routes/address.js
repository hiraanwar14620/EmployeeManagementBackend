...
addressRoutes.get('/', async (req, res) => {
    const dbRes = await executeQuery(`SELECT * FROM address`)
...
addressRoutes.post('/', async (req, res) => {
    ...
    const dbRes = await executeQuery(`INSERT INTO address ( "EmployeeId", "Address1", "Address2", "PostalCode", "City", "Phone1", "Phone2" ,"Province") values ('${EmployeeId}' , '${Address1}', '${Address2}', '${PostalCode}', '${City}', '${Phone1}','${Phone2}' ,'${Province}') returning *;`);
    res.status(200).json(dbRes.rows[0]);
});
